...
if (process.env.INITIALISED == "false"){
    console.log("erreur, le fichier .env n'est pas rempli, veuillez\
 entrer la commande '/make' pour le re-remplir");
 ...
 * @param {*} mode Le mode de connexion 0 pour la connexion , 1 inscription , 
 * 2 mdp oublie
 * 
...
async function operations(username,motdepasse,mail,mode) {
    ...
    let res = await client.query ("select * from utilisateur where username='"+
        username+"'");
    ...
    for(row of res.rows){
        if(row.mot_de_passe===motdepasse&&mode==0){//Le client est valide
            ...
            res = await client.query("update utilisateur set mot_de_passe = $1 \
                where username=$2",[motdepasse,username]);
    ...
    //Tentative d'inscription et aucun utilisateur qui a le meme pseudo 
    if(mode===1){
        await client.query("Insert into utilisateur values ($1,$2,$3)",
            [username,mail,motdepasse]);

        let res = await client.query("select id from utilisateur where \
            username=$1 and mot_de_passe=$2", [username, motdepasse]);

        if (res.rows.length == 0){
            console.log("impossible d'ajouter l'utilisateur dans la \
                base de donnée");
...
async function addReunion(req){
    try {
        ...
        for(let i =0;i<req.body.creneau.length;i++){
            ...
            tab_heure[i]=req.body.creneau[i][0].h+":"+req.body.creneau[i][0].m+
                ":00";

            console.log("heure"+tab_heure[i]);
            tab_heure_fin[i]=req.body.creneau[i][1].h+":"+
                req.body.creneau[i][1].m+":00";

            console.log("heure fin"+tab_heure_fin[i]);
        }
        const client = await pool.connect();
        let tab = [tab_heure,reunion_nom,username,date,tab_heure_fin, 
            red, blue, green, descr];

        let requete = "select id_reunion from reunion where heure=$1 and \
            nom_reunion=$2 and creator_username=$3 and date_reunion=$4";

        await client.query("insert into reunion (heure,nom_reunion, \
            creator_username, date_reunion,heure_fin, red, blue, green, descr) \
            values ($1,$2,$3,$4,$5,$6,$7,$8,$9)",tab);

        //Pas 2 reunion qui peuvent commencer au meme horraire
        let id = await client.query(requete,
            [tab_heure,reunion_nom,username,date]);
            
        await client.query("insert into participe values ($1,$2,$3)",
            [id.rows[0].id_reunion,username,2]);

        if(req.body.participe){
            await client.query("insert into participe values ($1,$2,$3)",
                [id.rows[0].id_reunion,req.body.mail,2]);
        }
        client.release();
...
/**
 * Renvoie les info de toutes les reunion ou l'utilisateur est invité ou 
 * participe
 * 
 * @param {*} mail
 * @returns 
...
async function getReunion(mail){
    const client = await pool.connect();
    let requete = "select reunion.* from reunion join participe on \
    participe.id_reunion = reunion.id_reunion"
    +" where participe.mail=$1";
    let res = await client.query(requete,[mail]);
    let res_invit = await client.query("select reunion.* from reunion join \
        invite on invite.id_reunion=reunion.id_reunion where invite.mail=$1",
        [mail]);
...
async function checkReunion(mail,creneau_list){
    ...
    for(let ind =0;ind<creneau_list.length;ind++){
        let res = await client.query("select reunion.heure_fin , reunion.heure \
            from reunion join participe on participe.id_reunion = \
            reunion.id_reunion where participe.mail=$1 and \
            reunion.date_reunion=$2",[mail,[creneau_list[ind][0].d]]);
        ...
        for(let row of res.rows){
            ...
            if((tmpMin < heureMin && tmpMax < heureMin)
                ||(tmpMax >heureMax && tmpMin > heureMax)
                ||(heureMin < tmpMin && heureMax < tmpMin)
                ||(heureMax>tmpMax && heureMin >tmpMax))
            {
                continue;
            }
            flag = false;
...
async function supParticipation(mail,id_reunion,createur){
    const client = await pool.connect();
    let res = await client.query("select role_reunion from participe where \
        mail=$1",[mail]);
    
    //IL faut supprimer la reunion directement
    if(res.rows[0]!==undefined && res.rows[0].role_reunion === 2){
        client.query("delete from reunion where id_reunion=$1",[id_reunion]);
    }else{
        client.query("delete from participe where id_reunion=$1 and mail =$2",
            [id_reunion,mail]);
    }
    client.release();
}
...
async function getInfoReunion(id_reunion){
    const client = await pool.connect();
    let res = await client.query("select role_reunion,mail from participe \
        where id_reunion=$1",[id_reunion]);
...
async function invitReunion(username,id,nom_reunion,remove) {
    ...
    if(!username.includes("@")){
        let res = await client.query("select mail from utilisateur where \
            username=$1",[username]);
    ...
    if(remove){await client.query("delete from invite where mail=$1",
        [adresse_mail]);}

    else{await client.query("insert into invite values($1,$2)",
        [id,adresse_mail]);}
    ...
    return await mail(process.env.MAIL,adresse_mail,
        ...
        +nom_reunion+
        " ? http://localhost:8080/invit/"+id+"/"+username)!==undefined;
}
...
async function reunion(id_reunion){
    ...
    const client = await pool.connect();
    let res = await client.query("select * from reunion where id_reunion=$1",
        [id_reunion]);
...
async function exists(username){
    const client = await pool.connect();
    let res = await client.query("select * from utilisateur where \
        username=$1",[username]);
...
async function importReunion(req){
    let id =0 ;
    if(!exists(req.body.organisateur)||!checkReunion(req.body.organisateur,
        req.body.date_debut,req.body.heure_debut,req.body.heure_fin)){

        console.log("PAS PASSE CHECK REUNION//reunion déja ajouté");
        return 2;
    }
    await addReunion(req).then(result=>id=result).catch(err=>{
        console.log("Reunion déja ajouté erreur : "+err.stack);id=-1;
    });
    if(id===-1) return 1;
    const client = await pool.connect();
    for(let participant_addr of req.body.invites){
        let res = await client.query("select username from utilisateur where \
            mail=$1",[participant_addr]);

        let pseudo = "";
        if(res.rows[0]!==undefined){
            pseudo = res.rows[0].username;
            //Normalement c'est pas possible sinon la reunion aurait déja 
            // été ajouté
            ...
            await client.query("insert into utilisateur(username,mail) values \
                ($1,$2)",[pseudo,participant_addr]);
        }
...
/**
 * Supprime l'utilisateur de la table invit et l'ajoute a la 
 * table participe si il la souhaite
 * 
 * @param {*} reponse 
 * @param {*} mail 
 * @param {*} id_reunion 
...
async function resInvit(reponse,mail,id_reunion,horraire){
    const client = await pool.connect();
    client.query("delete from invite where id_reunion=$1 and mail=$2",
        [id_reunion,mail]);

    if(reponse){
        client.query("insert into participe values($1,$2,0)",
            [id_reunion,mail,horraire]); 
    }
}
/**
...
async function checkInvit(id_reunion,mail){
    ...
    let res = await client.query("select * from invite where id_reunion=$1 \
        and mail=$2",[id_reunion,mail]);
    client.release();
...
app.post("/inscription",(req,res)=>{
    ...
    .then(resultats =>{
        if(resultats>0){
            ...
            res.json({result: false, message: "Erreur nom d'utilisateur deja \
                utilisé", id: -1});
        }else{
    ...
    .then(resultats =>{
        if(resultats>0){
            getInfoUser(resultats).then(result=>{
                ...
                res.json({connecte:true, message:"connecté!", 
                    id:resultats,mail:result.mail});}
            );
        }else if(resultats == -1){
            res.json({connecte:false, message:"nom d'utilisateur inconnu!", 
                id:-1});
                
        }else if(resultats == -2){
            res.json({connecte:false, message:"mot de passe incorrect", 
                id: -1});
...
app.post("/mdpOublie",(req,res)=>{
    ...
    .then(resultats =>{
        ...
        if (resultats < 0) {
            res.json({result: false, message: "Erreur: utilisateur \
                introuvable!"});
        }else{
            try {
                mail(process.env.MAIL,resultats.mail,"Reinitialisation Mot de \
                    passe",
                    "Cliquez sur ce lien pour reinitialisez votre mot de \
                    passe : http://localhost:8080/mdp/"+req.body.username);

                res.json({result: true});
            } catch (error){
                console.log(error);
                res.json({result: false, 
                    message:"impossible d'envoyer un mail!"});
            }
        }
    })
    .catch(erreur =>console.log(erreur.stack));
...
app.post('/creation',async (req,res)=>{
        ...
        addReunion(req).then(result=>res.json({result: true})).catch(err=>{
            console.log(err);res.json({result:false});
        });
});
...
app.post('/getReunion',(req,res)=>{
    ...
    .catch(error =>{
        console.log("Erreur ..."+error);res.json({result:"Erreur de mail"});
    });
});

//En gros envoyez via res.json toutes les reunion auquels l'utilisateur 
// peut accepter de participer
...
app.post('/invit',(req,res)=>{
    ...
    .then(result=>{
        console.log("L'envoie du mail c'est ...."+result);
...
app.get('/invit/:index/:mail',(req,res)=>{//L'id de la reunion 
        checkInvit(req.params.index,req.params.mail).then(result=>{
            if (result){
                reunion(req.params.index).then(result=>{
...
//recuperez toutes les info des reunion et supprimez les invit 
// quand ils ont clique sur le bouton du fichier invit
app.get('mdp/:username',(req,res)=>{
    console.log("Bonjour "+req.params.username+"Le site est pas encore \
        finis...");
});
